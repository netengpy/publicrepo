...
2. sdwan-lab-root-ca.crt from sdwan-lab-root-ca.key
...
+ Create sdwan-lab-root-ca.crt with sdwan-lab-root-ca.key
IMPORTANT: Organisation MUST match the Smart Account Organisation

vmanage:~$ 
openssl req -x509 -new -nodes -key sdwan-lab-root-ca.key -sha256 -days 1024 -subj "/C=UK/ST=LON/L=LON/O=mystacktracecom/CN=mystacktracecom/OU=SDWAN-LAB" -out sdwan-lab-root-ca.crt

+ Command to check the created certificate
openssl x509 -in sdwan-lab-root-ca.crt -text

+ Install sdwan-lab-root-ca.crt
exit
vmanage# request root-cert-chain install /home/admin/sdwan-lab-root-ca.crt
...
====================
vmanage# vshell
vmanage:~$ openssl x509 -req -in vmanage_csr -CA sdwan-lab-root-ca.crt -CAkey sdwan-lab-root-ca.key -CAcreateserial -out vmanage_crt -days 500 -sha256
...
Change Controller Certificate Authorization to Enterprise Root certificate and import sdwan-lab-root-ca.crt
***********
...
vBond
======
1. Install sdwan-lab-root-ca.crt from vManage that was created earlier
...
vBond# request root-cert-chain install scp://admin@192.168.1.1:/home/admin/sdwan-lab-root-ca.crt vpn 512

IF FAILS - 
copy/paste sdwan-lab-root-ca.crt from vmanage and install
...
vmanage:~$ openssl x509 -req -in vbond_csr -CA sdwan-lab-root-ca.crt -CAkey sdwan-lab-root-ca.key -CAcreateserial -out vbond_crt -days 500 -sha256
...
vsmart# request root-cert-chain install scp://admin@192.168.1.1:/home/admin/sdwan-lab-root-ca.crt vpn 512
...
vmanage:~$ openssl x509 -req -in vsmart_csr -CA sdwan-lab-root-ca.crt -CAkey sdwan-lab-root-ca.key -CAcreateserial -out vsmart_crt -days 500 -sha256
cat vsmart_crt
...
vEdge
======
1. Copy and Install sdwan-lab-root-ca.crt from vManage that was created earlier
2. Activate using chassis and token from vmanage

+ On vManage using “cat sdwan-lab-root-ca.crt” to see contents then create sdwan-lab-root-ca.crt file on
vEdge with same contents.
vshell
vi sdwan-lab-root-ca.crt
paste sdwan-lab-root-ca.crt from vManage

+ Install sdwan-lab-root-ca.crt on vEdge with command:
vedge# request root-cert-chain install /home/admin/sdwan-lab-root-ca.crt
